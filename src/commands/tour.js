...
module.exports = {
  name: "tour",
  description: "Меняет турнирный рейтинг пользователя на указанное число",
  ...
  async run(message, { args: [rawUser, rawAmount] }) {
    ...
    userData.tourRating += amount;

    await userData.save();

    message.channel.send({embeds: [
      new MessageEmbed()
        .setTitle("**Турнирный рейтинг изменён**")
        .addField("**Пользователь**", `**${user}**`, true)
        .addField("**Значение**", `**${amount}**`, true)
        .addField("**Новое значение**", `**${userData.tourRating}**`)
    ]}
    );
  }
};
